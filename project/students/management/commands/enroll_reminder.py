...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        users = User.objects.annotate(course_count=Count('courses_joined')).filter(courses_count=0, date_joined__lte=date_joined)
        for user in users:
            message = 'Dear {},\n\nWe noticed that you didn\'t enroll in any courses yet. What are you wating for?'.format(user.first_name)
